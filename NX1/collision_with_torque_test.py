...
class Ball:
    ...
    def update(self, ball_list, dt):
        ...
        if self.ang_vel.mag != 0:
            self.arrow.visible = True
            self.arrow.pos = [self.b.pos, self.b.pos + self.ang_vel]
        else:
...
arrow_b1 = curve()
arrow_b1.visible = False
arrow_b2 = curve()
arrow_b2.visible = False
...
while True:
    if b1 == None or (b1.stationary and b2.stationary) and not simulation_end:                
        ...
        b1 = Ball(m = 1, pos=vector(-10, ballrad, impact_param), vel=vector(200, 0, 0), ang_vel = vector(0,0,0), color=color.red, rad=ballrad)
        b1.arrow = arrow_b1
        ball_list.append(b1)
