...
rad_step = 0.016
max_impact_angle = np.pi/13
impact_angle = -max_impact_angle
...
while True:
    if b1 == None or (b1.stationary and b2.stationary) and not simulation_end:                
        ...
        b1 = Ball(m = 1, pos=vector(-4, ballrad, 0), vel=vector(np.cos(impact_angle), 0, np.sin(impact_angle))*200, ang_vel = vector(0,0,0), color=color.red, rad=ballrad)
        ball_list.append(b1)
        
        b2 = Ball(m = 1, pos=vector(edge_of_ramp, ballrad, 0), vel=vector(0, 0, 0), ang_vel = vector(0,0,0), color=color.blue, rad=ballrad)
        ball_list.append(b2)
        
        impact_angle += rad_step
        
        if impact_angle >= max_impact_angle:
            simulation_end = True
